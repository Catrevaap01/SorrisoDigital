 ...
 * Ver e editar dados do perfil, mudar senha e fazer logout
 */
...
const AdminProfileScreen: React.FC = () => {
  ...
  useFocusEffect(
    useCallback(() => {
      // Resetar form ao voltar para tela
      setEditando(false);
  ...
  const handleMudarSenha = async () => {
    if (!senhaAtual.trim()) {
      Toast.show({ type: 'error', text1: 'Senha atual é obrigatória' });
      return;
    }

    if (!novaSenha.trim()) {
      Toast.show({ type: 'error', text1: 'Nova senha é obrigatória' });
      return;
    }
    ...
    if (novaSenha.length < 6) {
      Toast.show({
        type: 'error',
        text1: 'Erro',
        text2: 'Nova senha deve ter no mínimo 6 caracteres',
      });
      return;
    }
    ...
    setProcessando(true);
    try {
      // TODO: Implementar mudança de senha com verificação da senha atual
      // Por enquanto apenas mostramos mensagem
      Toast.show({
        type: 'info',
        text1: 'Funcionalidade em desenvolvimento',
        text2: 'Mudança de senha será implementada em breve',
      });
    } finally {
      setProcessando(false);
    }
    ...
    <ScrollView style={styles.container}>
          ...
          <>
            ...
            <>
              ...
              <InfoCard
                icon="person"
                label="Nome"
                valor={profile?.nome || 'Não informado'}
              />
              <InfoCard
                icon="mail"
                label="Email"
                valor={profile?.email || 'Não informado'}
              />
              <InfoCard
                icon="call"
                label="Telefone"
                valor={profile?.telefone || 'Não informado'}
              />
              <InfoCard
                icon="location"
                label="Província"
                valor={profile?.provincia || 'Não informado'}
              />
              <InfoCard
                icon="calendar"
                label="Membro desde"
                valor={profile?.created_at ? new Date(profile.created_at).toLocaleDateString('pt-PT') : 'Não informado'}
              />
            </>
          )}
        </View>
      ...
      <View style={styles.section}>
        ...
        <TouchableOpacity
          ...
          onPress={() => setAbrirChangePassword(true)}
        >
      ...
      <View style={styles.section}>
        ...
        >
          <View style={styles.actionCardContent}>
            ...
            <View style={styles.actionCardText}>
              <Text style={[styles.actionCardTitle, { color: COLORS.danger }]}>
                Fazer logout
              </Text>
